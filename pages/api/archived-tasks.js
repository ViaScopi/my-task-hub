  ...
  if (req.method === "POST") {
    try {
      ...
      if (!snapshot || typeof snapshot !== "object") {
        return res.status(400).json({ error: "An archived task payload is required." });
      }

      const { source, original_id } = snapshot;
      if (!source || !original_id) {
        return res.status(400).json({
          error: "Archived task must include both source and original_id."
        });
      }
      ...
      const taskData = {
        ...
        status: snapshot.status || "archived",
      ...
      // Upsert the archived task (insert or update if exists)
      ...
      return res
        .status(status)
        .json({ error: error?.message || "Failed to save the archived task snapshot." });
    }
  }
